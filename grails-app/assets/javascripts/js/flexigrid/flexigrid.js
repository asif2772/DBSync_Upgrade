...
(function ($)
{
    ...
    $.addFlex = function (t, p)
    {
        ...
        var g = {
            ...
            rePosDrag: function ()
            {
				...
				(
			 	function ()
			 	{
			 	    ...
			 	    var x = g.bDiv.scrollLeft;
			 	    $('div:eq(' + n + ')', g.cDrag).css({ 'left': (p.width - cdpos) - 5 - 2 * x + 'px' }).attr('id', 'dr').show();
            ...
            dragMove: function (e)
            {
                ...
                if (this.colresize) //column resize
                {
                    ...
                    var diff = this.colresize.startX - e.pageX;
					     ...
					     $('thead tr:first th', g.hDiv).each
							(
							 	function ()
							 	{
							 	    ...
							 	    var idx = axis.substr(3);
							 	    if (idx < p.colModel.length)
							 	    {
							 	        td.align = this.align;
							 	        var column = p.colModel[idx]['name'];
							 	    ...
							 	    {
							 	        if (axis == 'col' + p.colModel.length)
							 	        {
							 	            for (var i = 0; i < p.buttons.length; i++)
							 	            {
							 	                if (p.buttons[i]['bclass'] == 'edit')
							 	                {
							 	                    ...
							 	                    {
							 	                        $(btnDiv).hover(function () { $(p.buttons[i]).addClass('fbOver'); }, function () { $(p.buttons[i]).removeClass('fbOver'); });
							 	                    }
							 	                    td.align = 'middle';
							 	                    $(td).append(btnDiv);
							 	                }
							 	            ...
							 	            if (axis == 'col' + (p.colModel.length + 1))
							 	            {
							 	                for (var i = 0; i < p.buttons.length; i++)
							 	                {
							 	                    if (p.buttons[i]['bclass'] == 'delete')
							 	                    {
            ...
            { //get latest data
                ...
                //var param = {page:p.newp, rp: p.rp, sortname: p.sortname, sortorder: p.sortorder, queryTxt: p.queryTxt, qtype: p.qtype};
                var param = [
					 { name: 'page', value: p.newp }
					, { name: 'rp', value: p.rp }
					, { name: 'sortname', value: p.sortname }
					, { name: 'sortorder', value: p.sortorder }
					, { name: 'queryTxt', value: p.queryTxt }
            ...
            doSearch: function ()
            {
                ...
                p.queryTxt = $('input[name=queryTxt]', g.sDiv).val();
        ...
        if (p.width != 'auto')
        {
            ...
            //                width += this.width;
            //            });
            //            g.gDiv.style.width = width + 170 + 'px';
            // p.width = g.gDiv.style.width;
        }
        ...
        if (p.buttons)
        {
            ...
            {
                if (p.buttons[i]['bclass'] == 'add' || p.buttons[i]['bclass'] == 'print' || p.buttons[i]['bclass'] == 'export')
                {
                    var btn = p.buttons[i];
                    if (!btn.separator)
                    {
                        ...
                        if (btn.onpress)
                        {
							...
							(
								function ()
								{
								    if (btn['bclass'] == 'print' || btn['bclass'] == 'export')
								        this.onpress(t, 1);
								    else
        ...
        if (p.usepager)
        {
            ...
            if (p.searchitems)
            {
                ...
                $(g.sDiv).append("<div class='sDiv2'> <select name='qtype'>" + sopt + "</select><input type='text' size='30' name='queryTxt' class='qsbox' /><input type='button' name='searchBtn' value='???' /></div>");

                $('input[name=queryTxt],select[name=qtype]', g.sDiv).keydown(function (e) { if (e.keyCode == 13) g.doSearch() });
                $('input[name=searchBtn]', g.sDiv).click(function () { g.doSearch(); });
                $(g.bDiv).after(g.sDiv);
