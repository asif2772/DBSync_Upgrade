...
function checkAcceptReconciliation(){
    var payType = $("#bankPaymentTypeId").val();
    if(!payType || payType.charAt(0)=="0"){
        showReconciliationAlertMsg("Please select type of Payment");
        return false;
    }
    var len = $('#bankReconciliationManual input[type=checkbox]:checked').length;
    if(len <= 0){
        showReconciliationAlertMsg("Please select some amount to process manual reconciliation");
        return false;
    }
    var cusVenSelData = $("#customerOrVendorId").val();
    if(cusVenSelData && cusVenSelData.substr(0,5)=="CUS_0"){
        showReconciliationAlertMsg("Please select Customer or Vendor");
        return false;
    }
...
function showReconciliationAlertMsg(recMsg){
    $("#showAlertMsg").html(recMsg);
    $("#showAlertMsg").show(500);
    setTimeout(function () {
        $("#showAlertMsg").hide('blind', {}, 1000);
    }, 5000)
}

function reconciliationVendorCustomerinfo(isSelectedVendor){
    ...
    if(isSelectedVendor == true || isSelectedVendor == null ){
        ...
        customerOrVednorid = $("#customerOrVendorId").find("option:contains('All Debtor')").val();
    }
